...
!---------------------------------------------------------------------
    ...
    USE Constants, ONLY :   qp, M_Sun, L_Sun, R_Sun, four_pi, sigma
    PRIVATE             ::  qp, M_Sun, L_Sun, R_Sun, four_pi, sigma
...
!           Gather the initial model data
        ...
        IMPLICIT NONE
        REAL(qp),       INTENT(OUT)     ::  Msolar, Lsolar
        REAL(qp),       INTENT(OUT)     ::  Ms, Teff, X, Y, Z
        REAL(qp),      INTENT(OUT)     ::  Ls, Rs, Rsolar
...
!           Get updated model input data
        ...
        IMPLICIT NONE
        REAL(qp),       INTENT(INOUT)   ::  Msolar, Lsolar
        REAL(qp),       INTENT(INOUT)   ::  Ms
        REAL(qp),      INTENT(INOUT)   ::  Rs, Rsolar, Ls
        REAL(qp),       INTENT(INOUT)   ::  Teff, X, Y, Z
...
!---------------------------------------------------------------------
    ...
    SUBROUTINE Too_Many_Zones(Msolar, Ms, M_r, Lsolar, Ls, L_r, r, Rs, Rsolar, Teff, X, Y, Z, P_0, T_0, &
...
!           Tell user that the maximum number of zones has been exceeded
        ...
        IMPLICIT NONE
        REAL(qp),       INTENT(IN)  ::  Msolar, Lsolar
        REAL(qp),       INTENT(IN)  ::  Ms, Teff, X, Y, Z
        REAL(qp),      INTENT(IN)  ::  Ls, Rs, Rsolar
        REAL(qp),      INTENT(IN)  ::  r
        REAL(qp),      INTENT(IN)  ::  L_r, M_r
        REAL(qp),      INTENT(IN)  ::  rho_0, kappa_0, P_0, T_0, epsilon_0
...
!           Tell the user the conditions at the surface and core of the completed model
        ...
        REAL(qp),      INTENT(IN)  ::  r
        REAL(qp),      INTENT(IN)  ::  L_r, M_r
        REAL(qp),      INTENT(IN)  ::  rho, kappa, epsilon, P, T
        REAL(qp),      INTENT(IN)  ::  rho_0, kappa_0, P_0, T_0, epsilon_0
        CHARACTER(1),   INTENT(IN)  ::  rc_flag
