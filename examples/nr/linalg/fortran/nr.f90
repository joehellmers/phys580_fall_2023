...
!BL
    ...
    END INTERFACE
    INTERFACE
        ! JH: Changed rank -> nrrank in order to avoid collision with new Fortran intrinsic
        FUNCTION nrrank(indx)
        USE nrtype
        INTEGER(I4B), DIMENSION(:), INTENT(IN) :: indx
        INTEGER(I4B), DIMENSION(size(indx)) :: nrrank
        END FUNCTION nrrank
    END INTERFACE
    INTERFACE
...
!BL
    ...
    END INTERFACE
    INTERFACE
        ! Change ran -> nrran in order to avoid intrinsic collision
        FUNCTION nrran(idum)
